...
if (currentWindow === "episode"){
  ...
  //------------------------------------------------------------------          PART II              -------------------------------------------------------------------------------------*/
	...
	var initialiseBar = function(seriesCounter, MakeBar) {
		seriesCounter = 0;
		MakeBar("series");
	}
	...
	if (decryption.decryptName === 'kissenc.min.js') {
		...
		// Only requires this script for decryption of the links
		$.getScript("/scripts/"+decryption.decryptName, function(){
			initialiseBar(window.seriesCounter, MakeBar);
